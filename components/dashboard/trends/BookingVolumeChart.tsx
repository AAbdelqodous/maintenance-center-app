...
import type { WeeklyBookingStat, TrendPeriod } from '@/types/trends';

interface BookingVolumeChartProps {
  data: WeeklyBookingStat[];
  insight: string;
  isLoading: boolean;
  period: TrendPeriod;
}

export function BookingVolumeChart({ data, insight, isLoading, period }: BookingVolumeChartProps) {
  ...
  if (isLoading) {
    return (
      <View style={styles.container}>
        <Text style={styles.chartTitle}>{t('trends.charts.bookings')}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {data.length === 0 ? (
        ...
        <>
          <BarChart
            data={data.map((w) => ({ value: w.total, label: w.periodLabel, frontColor: '#2196F3' }))}
            width={chartWidth}
            height={180}
