...
## Phase 4: User Story 4 — CENTER_OWNER Login Returns approvalStatus (Priority: P1)

**Goal**: CENTER_OWNER registration sets `PENDING_APPROVAL`; login response carries `approvalStatus`; REJECTED owners are blocked at login.

**Independent Test**: Register a CENTER_OWNER → verify email → login → confirm `approvalStatus=PENDING_APPROVAL` in response. Then reject via DB → login → confirm `403` with code `305`.
...
- [ ] T015 [US4] Update `AuthenticationService.register()` — resolve effective `userType` from request (default `CUSTOMER`; silently override `ADMIN`/`SUPER_ADMIN` to `CUSTOMER`); if `CENTER_OWNER`, call `user.setApprovalStatus(ApprovalStatus.PENDING_APPROVAL)` before `userRepository.save()` (depends on T013, T002, T006)
...
**Checkpoint**: Register CENTER_OWNER → activate via OTP → login returns `PENDING_APPROVAL`. CUSTOMER login returns `approvalStatus: null`. `GET /users/me` returns the user's profile.

---

## Phase 5: User Story 2 — Admin Lists Pending Registrations (Priority: P1)

**Goal**: Admin can retrieve a paginated list of CENTER_OWNER accounts awaiting approval.

**Independent Test**: Register two CENTER_OWNERs → admin calls `GET /admin/users/pending` → both appear; approve one via DB → list shows only the other.

- [ ] T018 [US2] Create `admin/AdminService.java` — `@Service @RequiredArgsConstructor @Slf4j`; add a static `UserResponse.from(User user)` factory method to `admin/UserResponse.java` for reuse, then implement `public Page<UserResponse> getPendingOwners(Pageable pageable)` using `userRepository.findByUserTypeAndApprovalStatus(CENTER_OWNER, PENDING_APPROVAL, pageable).map(UserResponse::from)` (depends on T004, T009)
...
## Phase 6: User Story 1 — Admin Approves a CENTER_OWNER (Priority: P1)

**Goal**: Admin can approve a specific CENTER_OWNER by ID; operation is idempotent.
...
- [ ] T020 [US1] Add `public UserResponse approveOwner(Integer id)` to `admin/AdminService.java` — load user by ID (throw `EntityNotFoundException` if missing), validate `userType == CENTER_OWNER` (throw `IllegalArgumentException` if not), set `approvalStatus = APPROVED`, save, return `toResponse()` (depends on T018)
...
## Phase 7: User Story 3 — Admin Rejects a CENTER_OWNER (Priority: P2)

**Goal**: Admin can reject a CENTER_OWNER with an optional reason; rejected owners are blocked at login.
...
**Checkpoint**: Reject a CENTER_OWNER → their login attempt returns `403` with bilingual error message and code `305`.
...
**Independent Test**: `GET /admin/users?type=CENTER_OWNER` returns only CENTER_OWNER accounts; `GET /admin/users` returns all types.
...
- After Phase 6 MVP → Platform can onboard CENTER_OWNERs end-to-end
