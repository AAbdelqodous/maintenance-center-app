...
export type CenterPermission =
  ...
  | 'MANAGE_PAYOUTS'
  // Spec 025 — inventory & parts. MANAGE_INVENTORY = catalog CRUD + receive/adjust stock
  // (OWNER, BRANCH_MANAGER); CONSUME_PARTS = add catalogued parts onto a quote
...
export const ROLE_PERMISSIONS: Record<CenterRole, CenterPermission[]> = {
  OWNER: [
    ...
    'RESPOND_TO_QUOTES', 'MANAGE_PAYOUTS', 'MANAGE_INVENTORY', 'CONSUME_PARTS',
  ],
  BRANCH_MANAGER: [
    ...
    'MANAGE_PAYOUTS', 'MANAGE_INVENTORY', 'CONSUME_PARTS',
  ],
  RECEPTIONIST: [
    'MANAGE_BOOKINGS', 'MANAGE_CHAT', 'VIEW_CALENDAR', 'VIEW_BOOKING_BASIC', 'VIEW_PRICE_LIST',
    'MANAGE_PRICING', 'MANAGE_OFFERS', 'RESPOND_TO_QUOTES', 'CONSUME_PARTS',
  ],
  TECHNICIAN: [
